...
| **Firmware tracker** | Monitors manufacturer firmware releases via GitHub API, RSS feeds, URL probes, and HTML scrapers. Matches against user equipment registry. Hub only. |
...
- **Hub mode** — API server, sync engine (hub side), RAG pipeline, auth (Supabase), real-time (Supabase), weather proxy, firmware tracker, MCP server
...
### Decision: Astro 5 + React 19 islands + Tailwind CSS + Ant Design 5
...
- **Tailwind CSS + Ant Design 5** — utility-first CSS with a comprehensive React component library with built-in dark mode, i18n, and accessibility. Ant Design Icons (@ant-design/icons).
...
|-----------|------|-----------|
| Hub → Spoke | Almanac updates, RAG database (regional subset), weather forecasts, tide data, chart updates, route library, user profile changes, firmware version database | On connect, then periodic |
...
- NMEA 2000 device enumeration — ISO Request (PGN 59904) for Product Information (PGN 126996) and Address Claim (PGN 60928). Returns manufacturer, model, serial, and firmware version for every device on the bus. Feeds equipment registry and firmware tracker.
...
equipment/
  devices/           — manufacturer, model, serial, install_date (per device)
  firmware/          — current_version, latest_version, update_available, changelog_url (per device)
```
...
| UI components | React 19 + Tailwind CSS + Ant Design 5 | Rich islands, dark mode, i18n, accessibility, progressive disclosure |
...
| Icons | Ant Design Icons (@ant-design/icons) | Consistent, Ant Design default |
