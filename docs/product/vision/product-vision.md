...
#### 6.11b Firmware Update Tracking

No service exists that tracks firmware updates across marine electronics manufacturers. Every boat owner must manually check 3-10 different manufacturer websites, apps, and portals to discover if updates are available. Victron has explicitly stated proactive firmware alerts are "not planned." This is a genuine gap.

- Firmware version database — maintained registry of latest firmware versions across manufacturers
- Personalized notifications — match user's registered equipment against new releases, notify via in-app feed and email digest
- Changelog aggregation — link to official changelogs, summarize key changes (new features, bug fixes, security patches)
- Community annotations — users report issues with specific firmware versions, flag compatibility problems
- VRM integration — for Victron users who link their VRM account, auto-detect devices and current firmware versions
- Compatibility warnings — flag known cross-device compatibility issues after firmware changes
- Update sequencing guidance — recommend order for multi-device updates (e.g. update GX before MPPTs)
- Risk indicators — flag high-risk updates (e.g. VE.Bus updates that reset settings to factory defaults)
- Equipment registry integration — shares the same device inventory used by maintenance scheduling, energy planning, and passage planning

**Phase 1 manufacturers (best data availability):**
- Victron Energy — VRM API, GitHub tags (Venus OS), blog RSS feed, Professional portal
- Garmin Marine — predictable monthly PDF bulletin URLs
- Raymarine — scrapeable download pages

**Phase 2 manufacturers:**
- Simrad / B&G / Lowrance (Navico)
- Furuno, Maretron, Mastervolt, Actisense, Digital Yacht

**Data acquisition:** Go workers on cron schedules — GitHub API monitoring, RSS polling, URL probing, HTML scraping. Metadata only (version numbers, dates, changelog summaries). Never host or redistribute firmware binaries — link to official manufacturer download pages.
...
- `docs/research/hardware/firmware-update-tracking.md` — Firmware update tracking feasibility study
